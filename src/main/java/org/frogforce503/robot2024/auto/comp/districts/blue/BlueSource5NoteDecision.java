...
public class BlueSource5NoteDecision extends AutoMode {
    ...
    public BlueSource5NoteDecision() {
        ...
        var shootingPosition = new Translation2d(3.715, 2.6);
        ...
        shootingPose = new Pose2d(shootingPosition, Rotation2d.fromDegrees(-36));
        // 2.5 arm, 0
        ...
        var grabEBranch = SwervePathBuilder.generate(5.5, 4.75, 3.5, 0,
            Waypoint.fromHolonomicPose(grabFirstTrunk.getFinalHolonomicPose()),
            new Waypoint(FieldConfig.getInstance().NOTE_E.plus(new Translation2d(Units.feetToMeters(0.0), Units.inchesToMeters(0))), new Rotation2d(), Rotation2d.fromDegrees(26))
        );
        ...
        var scoreD = SwervePathBuilder.generate(5.0, 4.75,
            ...
            Waypoint.fromHolonomicPose(shootingPose.transformBy(new Transform2d(new Translation2d(), Rotation2d.fromDegrees(0.0))))
        );

        var scoreE = SwervePathBuilder.generate(5.0, 4.75,
            Waypoint.fromHolonomicPose(grabEBranch.getFinalHolonomicPose()),
            new Waypoint(FieldConfig.getInstance().BLUE_STAGE_RIGHT.plus(new Translation2d(0, -1.0))),
            Waypoint.fromHolonomicPose(shootingPose.transformBy(new Transform2d(new Translation2d(), Rotation2d.fromDegrees(0.0))))
        );
        ...
        var grabFSecond = SwervePathBuilder.generate(5.5, 4.25, 0.0, 0,
            ...
            new Waypoint(FieldConfig.getInstance().BLUE_STAGE_LEFT.interpolate(FieldConfig.getInstance().BLUE_STAGE_RIGHT, 0.5).interpolate(FieldConfig.getInstance().BLUE_CENTER_STAGE, 0.25)),
            new Waypoint(FieldConfig.getInstance().NOTE_F.plus(new Translation2d(0.25, 0)), null, new Rotation2d())
        );

        var podiumShot = FieldConfig.getInstance().NOTE_I.plus(new Translation2d(-Units.feetToMeters(0.5), Units.feetToMeters(0.25)));
        var podiumPose = Waypoints.facingGoal(podiumShot);

        var scoreF = SwervePathBuilder.generate(5.5, 4.75, 0.0, 0,
            Waypoint.fromHolonomicPose(grabFSecond.getFinalHolonomicPose()),
            new Waypoint(FieldConfig.getInstance().BLUE_STAGE_LEFT.interpolate(FieldConfig.getInstance().BLUE_STAGE_RIGHT, 0.5).interpolate(FieldConfig.getInstance().BLUE_CENTER_STAGE, 0.5)),
            Waypoint.fromHolonomicPose(shootingPose.transformBy(new Transform2d(new Translation2d(), Rotation2d.fromDegrees(0)))),
            new Waypoint(FieldConfig.getInstance().NOTE_I.plus(new Translation2d(-0.5, -1))),
            new Waypoint(FieldConfig.getInstance().NOTE_I.plus(new Translation2d(-1.0, 0.25))),
        ...
        var bailFromDToE = SwervePathBuilder.generate(4.0, 3.5,
            ...
            new Waypoint(grabEBranch.getFinalHolonomicPose().getTranslation(), new Rotation2d(), Rotation2d.fromDegrees(45))
        );
    ...
    public Command routine() {
        return Commands.sequence(
            ...
            RobotContainer.hintAng(2.0, 0.0),
            ...
            Commands.parallel(
                ...
                Commands.sequence(
                    Commands.sequence(
                        ...
                        Commands.run(() -> {})
                    ).withTimeout(scorePath.get(CENTERLINE_NOTES.F).getTotalTimeSeconds() - 0.25),
