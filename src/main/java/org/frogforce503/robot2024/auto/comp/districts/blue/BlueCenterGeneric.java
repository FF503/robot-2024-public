...
public class BlueCenterGeneric extends AutoMode {
    ...
    public BlueCenterGeneric(CENTERLINE_NOTES note1, CENTERLINE_NOTES note2, CENTERLINE_NOTES note3) {
        this.note1 = note1;
        this.note2 = note2;
        this.note3 = note3;

        this.stableShotDistance = ShotPlanner.CENTRAL_SHOT_DISTANCE_BLUE - Units.feetToMeters(1.5);

        shootingPosition = Waypoints.facingGoal(
            new Translation2d(stableShotDistance, FieldConfig.getInstance().NOTE_J.getY())
        );

        Pose2d startingPose = setupPose(Waypoints.facingGoal(new Translation2d(FieldConfig.getInstance().BLUE_INITIATION_LINE - 0.5, FieldConfig.getInstance().NOTE_J.getY() - 0.5)));
        ...
        transit2 = FieldConfig.getInstance().BLUE_STAGE_LEFT.interpolate(FieldConfig.getInstance().BLUE_STAGE_RIGHT, 0.5);

        grabTrunk = SwervePathBuilder.generate(4.5, 3.75, 0, 3.0,
            Waypoint.fromHolonomicPose(shootingPosition),
            new Waypoint(transit1, transit1Direction, new Rotation2d()),
            new Waypoint(transit2, new Rotation2d(), new Rotation2d())
        );
    ...
    public Command routine() {
        return Commands.sequence(
            ...
            driveAndIntake(grabJ).withTimeout(grabJ.getTotalTimeSeconds() - 0.25), // shakiness at end
            RobotContainer.shootSequence(),
    ...
    private SIDE perspective(CENTERLINE_NOTES note) {
        return (note == CENTERLINE_NOTES.G) ? SIDE.LEFT : (note == CENTERLINE_NOTES.F ? SIDE.CENTER : SIDE.RIGHT);
    }

    private CENTERLINE_NOTES inversePerspective(SIDE side) {
        return (side == SIDE.LEFT) ? CENTERLINE_NOTES.G : (side == SIDE.CENTER ? CENTERLINE_NOTES.F : CENTERLINE_NOTES.E);
    }
    ...
    public Route getRoute() {
        return new Route(grabJ, grabTrunk, grab1, grab2, grab3);
    }
