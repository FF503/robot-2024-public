...
public class BlueAmp5CenterLineOFSZ extends AutoMode {

    private PlannedPath goOutForFirstButShoot, grabJ;
    ...
    public BlueAmp5CenterLineOFSZ (CENTERLINE_NOTES note1, CENTERLINE_NOTES note2, CENTERLINE_NOTES note3) {
        ...
        Pose2d secondShotPose = Waypoints.facingGoal(secondShotPos).transformBy(new Transform2d(new Translation2d(0, Units.feetToMeters(-1)), new Rotation2d()));
        ...
        var grabGSecond = SwervePathBuilder.generate(maxVel, maxAcc, 0.675, 0.0,
            ...
            new Waypoint(FieldConfig.getInstance().BLUE_STAGE_LEFT.plus(new Translation2d(0, 0.75)), null, new Rotation2d()),
        ...
        var comeBackScoreG = SwervePathBuilder.generate(maxVel, maxAcc,
            ...
            new Waypoint(FieldConfig.getInstance().BLUE_STAGE_LEFT.plus(new Translation2d(0, 0.75)), null, new Rotation2d()),
            Waypoint.fromHolonomicPose(secondShotPose.transformBy(new Transform2d(new Translation2d(), Rotation2d.fromDegrees(4))))
        );
        ...
        var comeBackScoreF = SwervePathBuilder.generate(maxVel, maxAcc,
            ...
            Waypoint.fromHolonomicPose(secondShotPose.transformBy(new Transform2d(new Translation2d(/*0.0, Units.feetToMeters(-3.0)*/), Rotation2d.fromDegrees(-5))))
        );
        ...
        grabJ = SwervePathBuilder.generate(3, maxAcc, 
            Waypoint.fromHolonomicPose(secondShotPose.plus(new Transform2d(new Translation2d(), new Rotation2d(Math.PI)))),
            // new Waypoint(secondShotPos.interpolate(FieldConfig.getInstance().NOTE_J, 0.5), null, new Rotation2d(Math.PI)),
            new Waypoint(FieldConfig.getInstance().NOTE_J, null, new Rotation2d(Math.PI))
        );
    ...
    public Command routine() {
        return Commands.sequence(
            ...
            drive(grabJ).alongWith(RobotContainer.intake())
            // RobotContainer.shootSequence()
        );
    }
    ...
    public Route getRoute() {
        return new Route()
            .addPaths(goOutForFirstButShoot, grabFirstPaths.get(this.note1), scorePaths.get(this.note1), grabSecondPaths.get(this.note2), scorePaths.get(this.note2), grabSecondPaths.get(this.note3), scorePaths.get(this.note3), grabJ);
    }
}